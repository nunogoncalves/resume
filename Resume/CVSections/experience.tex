%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Experience}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {iOS Developer} % Job title
    {Globestamp (\hyperlink{Globestamp}{https://globestamp.com}) } % Organization
    {Lisbon, Portugal} % Location
    {April. 2016 - PRESENT} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Responsible for the whole life cycle development of the Globestamp iOS app).}
        \item {Responsible for mentoring the back-end developer exposing the best API possible.}
        \item {Requirements analysis on both App and API behaviour.}
%		\link {https://itunes.apple.com/app/id1031240627}
      \end{cvitems}
    }

  \cventry
	{Software Developer} % Job title
    {Linkedcare (\hyperlink{linkedcare}{https://linkedcare.com}) } % Organization
	{Lisbon, Portugal} % Location
	{January. 2012 - March 2016} % Date(s)
	{
	 \begin{cvitems} % Description(s) of tasks/responsibilities
		\item {Responsible for the whole lifecycle development of the Globestamp iOS app.).}
		\item {Responsible for mentoring the backend developer exposing the best API possible.}
	\end{cvitems}
	}

  \cventry
    {Inov Contacto Trainee } % Job title
    {LatitudeN (\hyperlink{latitudeN}{https://latituden.com}) } % Organization
    {Darmstadt, Germany} % Location
    {January. 2011 - July 2011} % Date(s)
    {
	\begin{cvitems} % Description(s) of tasks/responsibilities
      	\item {Responsible for the whole lifecycle development of the Globestamp iOS app.).}
		 \item {Responsible for mentoring the backend developer exposing the best API possible.}
	\end{cvitems}
}

%---------------------------------------------------------
\end{cventries}
